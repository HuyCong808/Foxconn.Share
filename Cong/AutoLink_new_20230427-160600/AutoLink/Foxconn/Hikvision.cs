...
namespace Foxconn
{
    internal class Hikvision
    {
        ...
        /*
        - Load tat ca cac camera hikvision da ket noi
        - Open camera theo name (serial number, user defined name, index)
        - Set trigger mode = true
        - Set trigger software = true
        - Start grabbing


        - Lay anh
          + Excute Trigger Software
          + Grab Frame
         */
        ...
        public void DeviceListAcq()
        {
            ...
            if (0 != nRet)
            {
                Console.WriteLine("Enumerate devices fail!", 0);
                return;
            }
        ...
        public int Open(string name)
        {
            try
            {
                ...
                if (m_MyCamera == null)
                {
                    ...
                    return CreateDevice(name, m_nImageOnBuffer);
                }
            }
            catch (Exception ex)
            {
        ...
        public Bitmap GrabFrame(int timeout = 10000)
        {
            try
            {
                lock (_lockObj)
        ...
        private int CreateDevice(string Name, int m_nImageOnBuffer)
        {
            (bool bRet, MyCamera.MV_CC_DEVICE_INFO stDevInfo) = FindDevice(Name);
            if (!bRet)
            {
        ...
        public (bool, MyCamera.MV_CC_DEVICE_INFO) FindDevice(string Name)
        {
            for (int i = 0; i < m_stDeviceList.nDeviceNum; i++)
            {
                ...
                if (device.nTLayerType == MyCamera.MV_GIGE_DEVICE)
                {
                    ...
                    if (Name == stGigEDeviceInfo.chUserDefinedName)
                    {
                        return (true, device);
                    }
                }
                else if (device.nTLayerType == MyCamera.MV_USB_DEVICE)
                {
                    ...
                    if (Name == stUsb3DeviceInfo.chUserDefinedName)
                    {
                        return (true, device);
                    }
                }
        ...
        public int SetParameter(KeyName key, int value)
        {
            string keyString = GetStringKey(key);
            KeyType keyType = GetKeyType(keyString);
