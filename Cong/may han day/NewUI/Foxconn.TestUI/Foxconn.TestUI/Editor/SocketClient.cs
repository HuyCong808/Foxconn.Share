    ...
    public class SocketClient
    {
        ...
        public int Open(string host, int port)
        {
            try
            {
                ...
                if (_isConnected)
                {
                    ...
                    Console.WriteLine($"SocketClient.Open ({_host}:{_port}): Opened");
                    return 1;
                }
                else
                {
                    Console.WriteLine($"SocketClient.Open ({_host}:{_port}): Can not opened");
                    return 0;
                }
            }
            catch (Exception ex)
        ...
        public int SocketWriteData(string data)
        {
            try
            {
                ...
                {
                    Trace.WriteLine($"SocketClient.SocketWriteData ({_host}:{_port}): Can not opened");
                }
                return 0;
            }
            catch (Exception ex)
