...
namespace Foxconn.TestUI.Camera
{
    public class Hikvision : ICamera
    {
            ...
            try
            {
                ...
                for (int i = 0; i < m_stDeviceList.nDeviceNum; i++)
                {
                    ...
                    if (device.nTLayerType == MyCamera.MV_GIGE_DEVICE)
                    {
                        ...
                        {
                            Trace.WriteLine("GEV: " + gigeInfo.chUserDefinedName + " (" + gigeInfo.chSerialNumber + ")");
                        }
                        else
                        {
                    ...
                    else if (device.nTLayerType == MyCamera.MV_USB_DEVICE)
                    {
                        ...
                        {
                            Trace.WriteLine("U3V: " + usbInfo.chUserDefinedName + " (" + usbInfo.chSerialNumber + ")");
                        }
                        else
                        {
        ...
        private int CreateDevice(string userDefinedName, int m_nImageOnBuffer)
        {
            ...
            if (nRet != MyCamera.MV_OK)
            {
                Trace.WriteLine("Device create fail!");
                return nRet;
            }
            ...
            if (stDevInfo.nTLayerType == MyCamera.MV_GIGE_DEVICE)
            {
                ...
                if (nPacketSize > 0)
                {
                    ...
                    if (nRet != MyCamera.MV_OK)
                    {
                        Trace.WriteLine("Warning: Set Packet Size failed!");
                        return nRet;
                    }
                }
                else
                {
                    Trace.WriteLine("Warning: Get Packet Size failed!");
                    return -1;
                }
            }
