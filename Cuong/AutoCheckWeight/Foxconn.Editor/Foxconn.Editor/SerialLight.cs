    ...
    public class SerialLight
    {
        ...
        public int Open(string portName)
        {
            try
            {
                ...
                {
                    _isConnected = true;
                    Logger.Current.Info($"SerialLightt.Open ({_portName}): Opened ");
                    return 1;
                }
                else
                {
                    Logger.Current.Info(($"SerialLight.Open ({_portName}): Can not Open"));
                }
                return 0;
            }
            catch (Exception ex)
        ...
        public void Close()
        {
            try
            {
                ...
                _serialPort.Dispose();
                _serialPort.Close();
                Logger.Current.Info($"SerialLight.Close ({_portName}): Closed");
            }
            catch (Exception ex)
            {
                Logger.Current.Error(ex.Message);
        ...
        private void SerialDataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                if (data.Length > 0) ;
                {
                    _dataReceive = data;
                    _serialPort.DiscardInBuffer();
                    Logger.Current.Info($"SerialLight.SerialDataReceived ({_portName}): {data}");
                }
            }
            catch (Exception ex)
            {
        ...
        public int SendData(string data)
        {
            try
            {
                ...
                Logger.Current.Info($"SerialLight.SendData ({_portName}) : {data}");
                return 1;
            }
            catch (Exception ex)
            {
