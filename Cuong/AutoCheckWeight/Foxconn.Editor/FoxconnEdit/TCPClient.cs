    ...
    public class TCPClient
    {
        ...
        public int Open(string host, int port)
        {
            try
            {
                ...
                if (_isConnected)
                {
                    ...
                    logger.Info($"TcpClient.Open ({_host}:{_port}): Opened");
                    Console.WriteLine($"TcpClient.Open ({_host}:{_port}): Opened");
                    return 1;
                }
                else
                {
                    logger.Info($"TcpClient.Open ({_host}:{_port}): Can not opened");
                    Console.WriteLine($"TcpClient.Open ({_host}:{_port}): Can not opened");
                    return 0;
                }
            }
            catch (Exception ex)
        ...
        public void Close()
        {
            try
            {
                ...
                logger.Info($"TcpClient.Close ({_host}:{_port}): Closed");
            }
            catch (Exception ex)
            {
                    ...
                    try
                    {
                        if (_isConnected)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _dataReceived = data;
                                    logger.Info($"TcpClient.SocketDataReceived ({_host}:{_port}): {data}");
                                    Console.WriteLine($"TcpClient.SocketDataReceived ({_host}:{_port}): {data}");
                                }
                            }
                        }
                    }
        ...
        public int SocketWriteData(string data)
        {
            try
            {
                if (_isConnected)
                {
                    ...
                    _stream.Flush();
                    _stream.FlushAsync();
                    logger.Info($"TcpClient.SocketWriteData ({_host}:{_port}): {data}");

                    return 1;
                }
                else
                {
                    logger.Info($"TcpClient.SocketWriteData ({_host}:{_port}): Can not opened");
                }
                return 0;
            }
            catch (Exception ex)
