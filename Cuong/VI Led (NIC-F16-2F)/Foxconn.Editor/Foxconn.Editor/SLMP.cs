...
namespace Foxconn.Editor
{
    public class SLMP
    {
        ...
        public int GetFlag(string device)
        {
            ...
            if (value == 1)
            {
                Trace.WriteLine($"Get {device} ({_host}:{_port}): {value}");
                nRet = SetBitDevice(device, 0);
                if (nRet == 1)
                {
                    Trace.WriteLine($"Set {device} ({_host}:{_port}): 0");
                }
                else
                {
                    Trace.WriteLine($"Set {device} ({_host}:{_port}): {value} (ERROR)");
                }
                return nRet;
            }
            return -1;
        ...
        public int SetDevice(string device, int value)
        {
            ...
            if (nRet == 1)
            {
                Trace.WriteLine($"Set {device} ({_host}:{_port}): {value}");
            }
            else
            {
                Trace.WriteLine($"Set {device} ({_host}:{_port}): {value} (ERROR)");
